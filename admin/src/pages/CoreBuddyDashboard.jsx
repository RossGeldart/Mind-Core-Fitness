...
export default function CoreBuddyDashboard() {
  ...
  const [todayHealthData, setTodayHealthData] = useState(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getDoc(doc(db, 'healthData', `${clientData.id}_${todayStr}`)),
    ]).then(([ciSnap, hdSnap]) => {
      if (ciSnap.exists()) setTodayCheckIn(ciSnap.data());
      if (hdSnap.exists()) setTodayHealthData(hdSnap.data());
    }).catch(() => {});
  }, [clientData?.id]);
