...
export default function CoreBuddyNav({ active }) {
  ...
  return (
    <nav className="block-bottom-nav">
      ...
      <button className={`block-nav-tab${active === 'nutrition' ? ' active' : ''}${!isPremium ? ' nav-locked' : ''}`} onClick={() => navigate(isPremium ? '/client/core-buddy/nutrition' : '/upgrade')}>
        <div className="nav-icon-wrap">
          ...
          {!isPremium && <svg className="nav-lock-icon" width="10" height="10" viewBox="0 0 24 24" fill="currentColor" stroke="none"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>}
        </div>
        <span>Nutrition</span>
      </button>
      <button className={`block-nav-tab${active === 'buddies' ? ' active' : ''}${!isPremium ? ' nav-locked' : ''}`} onClick={() => navigate(isPremium ? '/client/core-buddy/buddies' : '/upgrade')}>
        <div className="nav-icon-wrap">
